...
train['standard'] = [textstat.text_standard(x, float_output=True) for x in train['excerpt']]
...
axs[0,0].hist(train['target'],bins=12)
axs[0,0].set_title('Target Score')

axs[0,1].hist(train['words'],bins=12)
axs[0,1].set_title('Wordcount')

axs[1,0].hist(train['unique'],bins=12)
axs[1,0].set_title('Unique wordcount')

axs[1,1].hist(train['sents'],bins=12)
axs[1,1].set_title('Sentence count')

# set labels
plt.setp(axs[:, 0], ylabel='Sample count')
